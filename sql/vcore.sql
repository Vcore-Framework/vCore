...
-- Add foreign keys for player_sessions
ALTER TABLE `player_sessions` 
    ADD CONSTRAINT `fk_sessions_player` 
    FOREIGN KEY (`citizenid`) REFERENCES `players` (`citizenid`) ON DELETE CASCADE;

-- Add foreign keys for session_logs
ALTER TABLE `session_logs` 
    ADD CONSTRAINT `fk_session_logs_player` 
    FOREIGN KEY (`citizenid`) REFERENCES `players` (`citizenid`) ON DELETE CASCADE;

-- Add foreign keys for currency_logs
ALTER TABLE `currency_logs` 
    ADD CONSTRAINT `fk_currency_logs_player` 
    FOREIGN KEY (`citizenid`) REFERENCES `players` (`citizenid`) ON DELETE CASCADE;

-- Add foreign keys for profession_history
ALTER TABLE `profession_history` 
    ADD CONSTRAINT `fk_profession_history_player` 
    FOREIGN KEY (`citizenid`) REFERENCES `players` (`citizenid`) ON DELETE CASCADE;

-- Add foreign key for organization_members citizenid
ALTER TABLE `organization_members` 
    ADD CONSTRAINT `fk_org_members_player` 
    FOREIGN KEY (`citizenid`) REFERENCES `players` (`citizenid`) ON DELETE CASCADE;
...
DROP VIEW IF EXISTS `view_active_players`;
DROP VIEW IF EXISTS `view_wealth_leaderboard`;

-- Active Players View
CREATE VIEW `view_active_players` AS
SELECT 
    p.citizenid,
    p.identifier,
...
CREATE VIEW `view_wealth_leaderboard` AS
SELECT 
    citizenid,
...
CREATE PROCEDURE sp_get_player_stats(IN p_citizenid VARCHAR(50))
BEGIN
    SELECT 
        p.*,
